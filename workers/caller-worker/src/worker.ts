...
iii.registerFunction(
  ...
  async (payload: any) => {
    logger.info("Received HTTP trigger payload:", payload);
    
    let body: any = {};
    if (payload.body) {
      body = payload.body;
    } else if (payload.request_body) {
      try {
        const bodyBuffer = await payload.request_body.readAll();
        const bodyStr = bodyBuffer.toString('utf-8');
        logger.info("Parsed HTTP body string:", bodyStr);
        if (bodyStr) {
          body = JSON.parse(bodyStr);
        }
      } catch (err) {
        logger.error("Failed to parse HTTP request body:", err);
      }
    } else {
      body = payload;
    }
