...
def run_inference_handler(payload: Dict[str, str | List[Dict[str, Any]]]) -> Dict[str, Any]:
    print("PYTHON INFERENCE PAYLOAD RECEIVED:", payload, flush=True)
    if payload is None:
        print("PYTHON INFERENCE PAYLOAD IS NONE", flush=True)
        return {"error": "payload is None"}
    messages = payload.get("messages", [])
    print("PYTHON INFERENCE MESSAGES EXTRACTED:", messages, flush=True)

    text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
    print("PYTHON INFERENCE RENDERED TEXT:", text, flush=True)
